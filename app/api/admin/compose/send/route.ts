...
//
// Validates → renders Markdown → loads each recipient's email and
// unsubscribe_token → sends via Resend in batches → writes a mailers
// audit row.
//
// Re-checks unsubscribed_at IS NULL at send time so a race where
// someone unsubs between picker load and send doesn't email them.
...
export async function POST(req: NextRequest) {
  ...
  // Re-fetch recipients server-side. We filter unsubscribed_at IS
  // NULL again here in case someone unsubbed between picker load
  // and send. PostgREST `in.(...)` filter handles the bulk lookup.
  ...
  try {
    ...
    console.error('[compose/send] recipient lookup failed:', e instanceof Error ? e.message : e)
  ...
  try {
    ...
    console.error('[compose/send] audit insert failed:', e instanceof Error ? e.message : e)
  }
